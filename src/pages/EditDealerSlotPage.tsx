...
const EditDealerSlotPage: React.FC = () => {
  ...
  // Original slot data for reference
  ...
  useEffect(() => {
    const fetchSlotDetails = async () => {
      try {
        ...
        // Format expiry date for the input field (YYYY-MM-DD)
        let formattedExpiryDate = '';
        if (slot.expiryDate) {
          const date = new Date(slot.expiryDate);
        ...
        setFormData({
          ...
          dealerPrice: slot.dealerPrice || 0,
          loyaltyPoints: slot.loyaltyPoints || 0,
          expiryDate: formattedExpiryDate,
          status: slot.status || 'active',
          redemptionRules: {
            pointsRequired: slot.redemptionRules?.pointsRequired || 0,
            discountPercentage: slot.redemptionRules?.discountPercentage || 0,
            additionalBenefits: slot.redemptionRules?.additionalBenefits?.length > 0 
              ? slot.redemptionRules.additionalBenefits 
              : ['']
          }
        });
  ...
  const handleQuantityChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (!isNaN(quantity) && quantity > 0) {
      // Additional validation would happen on the server
      setFormData(prevData => ({
        ...prevData,
        quantity
      }));
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      await axios.put(`/api/dealer-slots/${slotId}`, payload);
      
      navigate(`/dashboard/dealer-slots/${slotId}`);
    } catch (err: any) {
      setError(err.response?.data?.message || 'Failed to update dealer slot');
      console.error('Error updating dealer slot:', err);
    } finally {
  ...
  return (
    <div className="container mx-auto px-4">
      <div className="mb-6">
        <h1 className="text-2xl font-bold text-gray-900">Edit Dealer Slot</h1>
        <p className="text-gray-600">
          Update your dealer slot details
        </p>
      </div>
      ...
      <form onSubmit={handleSubmit} className="bg-white shadow-md rounded-lg p-6">
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          ...
          <div>
            ...
            <input
              type="text"
              value={originalSlot.originalProduct?.name || ''}
              className="w-full px-3 py-2 border border-gray-300 rounded-md bg-gray-100 cursor-not-allowed"
              disabled
            />
            <p className="text-xs text-gray-500 mt-1">Original product cannot be changed</p>
          </div>
          ...
          <div>
            ...
            <input
              ...
              min={originalSlot.quantity - originalSlot.availableQuantity}
              className="w-full px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-blue-500"
              required
            />
            {originalSlot.quantity !== originalSlot.availableQuantity && (
              <p className="text-xs text-gray-500 mt-1">
                Cannot reduce below {originalSlot.quantity - originalSlot.availableQuantity} units (already sold)
              </p>
            )}
          </div>
        ...
        <div className="mt-8 flex justify-end">
          <button
            type="button"
            onClick={() => navigate(`/dashboard/dealer-slots/${slotId}`)}
...
export default EditDealerSlotPage; 